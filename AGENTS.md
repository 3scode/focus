...
- **Auth:** Clerk (`@clerk/nextjs`). `ClerkProvider` di `layout.tsx`, middleware di `middleware.ts`. Sign-in/sign-up catch-all di `src/app/sign-in/[[...rest]]/` dan `src/app/sign-up/[[...rest]]/`.
- **State management:** React Context saja — tiga provider di `src/app/providers.tsx`:
  - `AuthProvider` → pakai dari `@/store/auth` (wrapper Clerk)
...
- `src/lib/` — utilitas: `storage.ts` (IndexedDB — termasuk habits & habitRecords), `time.ts`, `constants.ts`, `register-sw.ts`
